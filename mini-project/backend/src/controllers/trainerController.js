...
exports.getPlayers = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT p.*, COALESCE(pa.readiness, 0) AS readiness_score
       FROM players p
       LEFT JOIN (
         SELECT DISTINCT ON (player_id) player_id, readiness
...
// --- Team Analytics: save & fetch last team ---

exports.saveTeam = async (req, res) => {
  try {
    const { teamA, teamB, bestTeam } = req.body;
    ...
    await pool.query(
      `INSERT INTO team_selections (team_a, team_b, best_team) VALUES ($1, $2, $3)`,
      [JSON.stringify(teamA), JSON.stringify(teamB), JSON.stringify(bestTeam)]
    );
...
exports.getLastTeam = async (req, res) => {
  try {
    // Create the table if it doesn't exist
    await pool.query(`
      CREATE TABLE IF NOT EXISTS team_selections (
    ...
    const result = await pool.query(
      `SELECT * FROM team_selections ORDER BY created_at DESC LIMIT 1`
    );
    ...
    console.error("Get last team error:", err);
    res.status(500).json({ message: "Failed to fetch last team" });
  }
};